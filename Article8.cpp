...
HRESULT InitWindow( HINSTANCE hInstance, int nCmdShow )
{
    ...
    g_hWnd = CreateWindow( L"Article8", L"Кутасевич 924403: Загрузка меша", WS_OVERLAPPEDWINDOW,
...
HRESULT InitGeometry()
{
	...
	//Загрузка меша
	g_pMesh1=new MeshFromObj(g_pd3dDevice,g_pImmediateContext,"platonic.obj");


    //// Загрузка текстуры
    hr = D3DX11CreateShaderResourceViewFromFile( g_pd3dDevice, L"platonicColor.png", NULL, NULL, &g_pTextureRV, NULL );
    if( FAILED( hr ) )
        return hr;
...
void Render()
{
	...
	XMVECTOR Eye = XMVectorSet(1.0f, 1.0f, 15.0f, 0.0f );
